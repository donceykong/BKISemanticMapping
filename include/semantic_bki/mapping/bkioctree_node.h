...
namespace semantic_bki {
    ...
    public:
        ...
        Semantics() : ms(std::vector<float>(num_class, prior)), state(State::UNKNOWN),
                      osm_building(0.f), osm_road(0.f), osm_grassland(0.f), osm_tree(0.f), osm_parking(0.f), osm_fence(0.f) { classified = false; }

        Semantics(const Semantics &other) : ms(other.ms), state(other.state), semantics(other.semantics),
                      osm_building(other.osm_building), osm_road(other.osm_road), osm_grassland(other.osm_grassland), osm_tree(other.osm_tree), osm_parking(other.osm_parking), osm_fence(other.osm_fence) { }
        ...
        /// OSM prior values in [0,1]: building, road, grassland, tree, parking, fence (Euclidean signed-distance-based).
